...
import { getChatList, getSingleChat } from '../../../features/users/Common/inboxSlice'
    ...
    return (
        <div>
            <div className="w-full p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                <div className="flex flex-col md:flex-row gap-5 items-start md:items-center my-5 pb-5 border-b border-gray-500 dark:border-gray-500">
                    ...
                    <ul role="list" className="divide-y divide-gray-200 dark:divide-gray-700">
                        {
                            isChatListLoading ? <LoaderCard /> :
                                !chatList || chatList.length <= 0 ? <ListEmpty /> :
                                    chatList.map((chat, i) => (
                                        role === "learner" ? <li className="py-3 sm:py-4" key={i}>
                                            <div className="flex items-center space-x-4 cursor-pointer" onClick={() => handleClick(chat.chatId)}>
                                                ...
                                                </div>
                                                {
                                                    chat.users.map((user, i) => (
                                                        (user.userId == userId && user.unReadMessageCount > 0) && <span key={i} className="bg-blue-100 text-blue-800 text-sm font-semibold inline-flex items-center py-1.5 px-2 rounded-full dark:bg-gray-700 dark:text-blue-400">
                                                            {user.unReadMessageCount}
                                                        </span>
                                                    ))
                                                }
                                            </div>
                                        </li> : chat.latestMessage && chat.latestMessage._id !== "" && <li className="py-3 sm:py-4" key={i}>
                                            <div className="flex items-center space-x-4 cursor-pointer" onClick={() => handleClick(chat.chatId)}>
                                                <div className="flex-shrink-0">
                                                    <img className="w-8 h-8 rounded-full" src={!chat.profilePicture || chat.profilePicture === "Not Available" ? blankProfilePic : chat.profilePicture} alt="Bonnie image" />
                                                </div>
